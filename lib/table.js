...
Table.prototype.insert = function(data, cb) {
  ...
  query(sql, data, {write: true}, function(err, rs) {
    ...
    // save to cache
    self.db._opts.cache.set(cacheKey, JSON.stringify(rs[0]), function(err) {
      if (err) return cb(err)
      cb(null, row)
    })
  })
...
Table.prototype.getPrimaryKeyWhereClause = function(key) {
  ...
  self.primaryKey.forEach(function(field, i) {
    ...
    criteria += iq + self.name + iq + '.' + iq + field + iq + " = " + "'" + key[field] + "'"
  })
  return criteria;
}
...
Table.prototype.save = function(data, cb) {
  var self = this
  var row = self.instantiateRow(data)
  row.persist(cb)
  // // get primary key from data
  // var pk = {}
  // self.primaryKey.forEach(function(field) {
  //   pk[field] = data[field]
  // })
  // console.log('pk:', pk)
  // // check to see if this primary key already exists
  // self.get(pk, function(err, row) {
  //   if (err) {
