  ...
  if (opts) {
    ...
    if (isArray(opts.where)) {
      ...
      Object.keys(opts.where).forEach(function(property) {
        ...
        if (opts.where[property] === null) {
          return sql.push('AND ' + iq + property + iq + " IS NULL")
        }
        if (Array.isArray(opts.where[property])) {
          sql.push('AND ' + iq + property + iq + " IN (:" + property + ")")
        } else {
    ...
    if (!opts.count) {
      ...
      // limit
      if (isInteger(Number(opts.limit))) {
        sql.push('LIMIT ' + opts.limit)
      }
      // offset
      if (isInteger(Number(opts.offset))) {
