...
Table.prototype.insert = function(data, cb) {
  ...
  query(sql, data, {write: true}, function(err, rs) {
    ...
    var cacheKey = self.getPrimaryKeyWhereClause(row.getPrimaryKey())
    self.orm._opts.cache.set(cacheKey, rs[0], function(err) {
      if (err) return cb(err)
      cb(null, row)
    })
  })
...
Table.prototype.get = function(key, cb) {
  ...
  self.orm._opts.cache.get(cacheKey, function(err, val) {
    if (err) return cb(err)
    if (val) {
...
Table.prototype.invalidateMemo = function(key) {
  var self = this
  var cacheKey = self.getPrimaryKeyWhereClause(key)
