...
Table.prototype.get = function(key, cb) {
  ...
  if (memoMs) {
    if (memo[memoKey] && memo[memoKey].timestamp + memoMs > Date.now()) {
      // if this query is already in progress don't run it again
      var interval = setInterval(function() {
        if (!memo[memoKey].queryInProgress) {
          clearInterval(interval)
          instantiateRow(memo[memoKey].data)
        }
      }, 1)
      return
    }
...
Table.prototype.invalidateMemo = function(key) {
  var self = this
  var memoKey = self.getPrimaryKeyWhereClause(key)
  delete memo[memoKey]
}
