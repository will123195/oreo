...
Row.prototype.persist = function(cb) {
  ...
  table.columns.forEach(function(column) {
    if (typeof data[column.column_name] !== 'undefined') {
      properties.push(column.column_name)
    }
  })
  var set = []
  ...
  var query = self._meta.orm.execute

  var db = table.orm
  var pk = self.getPrimaryKey()
  var pkWhere = false
  console.log(table.name + ':', pk)
  var isInsert = false
  ...
  //console.log('persist', table.name, table)
  var result
  
  async.series([
    // begin transaction
    function(next) {
      console.log('begin', table.name)
      query('BEGIN', next)
    },
    // save 1-to-1 nested objects
    function(next) {
      async.eachSeries(properties, function(property, callback) {
        if (table.fk[property]) {
          var foreignTable = db[table.fk[property].foreignTable]
          var foreignRow = new Row(self[property], foreignTable)
          foreignRow.persist(callback)
        } else {
          callback(null)
        }
      }, next)
    ...
    // commit transaction
    function(next) {
      console.log('commit', table.name)
      query('COMMIT', next)
    }
  ], function(err) {
    if (err) {
      // rollback transaction
      console.log('rollback', table.name, err)
      query('ROLLBACK', function(rollback_err) {
        if (rollback_err) return cb(rollback_err)
        cb(err)
      })
      return
    }
